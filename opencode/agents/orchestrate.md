...
5. If the task needs documentation creation or updates, especially `docs/` markdown -> `document`
6. If the task needs durable project knowledge or memory updates -> `archive`
7. If the task needs external docs, APIs, package behavior, or architecture facts -> `research`
8. If the task is UI/UX, layout, interaction, accessibility, or visual design -> `design`
9. If the task is broad codebase discovery or parallel search -> `general` or `explore`
